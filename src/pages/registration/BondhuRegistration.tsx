...
export default function BondhuOnboarding() {
  ...
  const handleFileChange = (field: 'photo' | 'college_id' | 'aadhaar', file: File | null) => {
    ...
    if (/[\u4e00-\u9fa5]/.test(file.name)) {
      ...
      console.error(`❌ Filename contains Chinese characters: ${file.name}`);
      return;
    }
    ...
    reader.onloadend = () => {
      ...
      console.log(`✅ Preview generated for ${field}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        if (formData.step2.photo) {
          console.log('📸 Uploading photo...', {
            fileName: formData.step2.photo.name,
            fileSize: formData.step2.photo.size,
            fileType: formData.step2.photo.type
          });
          photoUrl = await uploadDocument(user.id, formData.step2.photo, 'photo');
          console.log('✅ Photo uploaded successfully:', photoUrl);
        } else {
          console.warn('⚠️ No photo file selected');
        }
      } catch (error: any) {
      ...
      try {
        if (formData.step2.college_id) {
          console.log('🎓 Uploading college ID...', {
            fileName: formData.step2.college_id.name,
            fileSize: formData.step2.college_id.size,
            fileType: formData.step2.college_id.type
          });
          collegeIdUrl = await uploadDocument(user.id, formData.step2.college_id, 'college_id');
          console.log('✅ College ID uploaded successfully:', collegeIdUrl);
        } else {
          console.warn('⚠️ No college ID file selected');
        }
      } catch (error: any) {
        console.error('❌ College ID upload error:', error);
        throw new Error(`Failed to upload college ID: ${error.message || 'Unknown error'}`);
      }

      try {
        if (formData.step2.aadhaar) {
          console.log('🆔 Uploading Aadhaar...', {
            fileName: formData.step2.aadhaar.name,
            fileSize: formData.step2.aadhaar.size,
            fileType: formData.step2.aadhaar.type
          });
          aadhaarUrl = await uploadDocument(user.id, formData.step2.aadhaar, 'aadhaar');
          console.log('✅ Aadhaar uploaded successfully:', aadhaarUrl);
        } else {
          console.warn('⚠️ No Aadhaar file selected');
        }
      } catch (error: any) {
        console.error('❌ Aadhaar upload error:', error);
        throw new Error(`Failed to upload Aadhaar: ${error.message || 'Unknown error'}`);
      }
      ...
      // Insert document records into documents table BEFORE refreshing profile
      ...
      if (photoUrl && formData.step2.photo) {
        documentRecords.push({
          user_id: user.id,
          document_type: 'photo',
          file_url: photoUrl,
          file_name: formData.step2.photo.name,
          file_size: formData.step2.photo.size
        });
      }
      
      if (collegeIdUrl && formData.step2.college_id) {
        documentRecords.push({
          user_id: user.id,
          document_type: 'college_id',
          file_url: collegeIdUrl,
          file_name: formData.step2.college_id.name,
          file_size: formData.step2.college_id.size
        });
      }
      
      if (aadhaarUrl && formData.step2.aadhaar) {
        documentRecords.push({
          user_id: user.id,
          document_type: 'aadhaar',
          file_url: aadhaarUrl,
          file_name: formData.step2.aadhaar.name,
          file_size: formData.step2.aadhaar.size
        });
      }
      ...
      if (documentRecords.length > 0) {
        try {
          ...
          if (docsError) {
            ...
            throw new Error(`Failed to save document records: ${docsError.message}`);
          }
          
          console.log('✅ Documents saved successfully:', documentRecords.length, 'documents');
        } catch (error: any) {
          console.error('Documents table error:', error);
          throw new Error(`Failed to save document records: ${error.message || 'Unknown error'}`);
        }
      }

      // Refresh profile to update AuthContext with new role and registration status
      ...
      toast.success('🎉 Registration completed successfully!', {
        description: 'Welcome to BondhuApp! Redirecting to your dashboard...',
        duration: 3000,
      });
      
      // Redirect after a short delay to ensure profile is updated
      setTimeout(() => {
        console.log('🚀 Redirecting to Bondhu dashboard...');
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 py-8 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex justify-center mb-8 animate-fade-in">
          <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-secondary/20 bg-white max-w-md">
            <img 
              src="/role-select.jpg" 
              alt="Select Your Role - Join as Bondhu Helper" 
